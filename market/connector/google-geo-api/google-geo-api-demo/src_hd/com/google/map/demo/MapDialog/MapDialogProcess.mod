[Ivy]
1790BE16C1C22AD7 9.2.0 #module
>Proto >Proto Collection #zClass
Ms0 MapDialogProcess Big #zClass
Ms0 RD #cInfo
Ms0 #process
Ms0 @AnnotationInP-0n ai ai #zField
Ms0 @TextInP .type .type #zField
Ms0 @TextInP .processKind .processKind #zField
Ms0 @TextInP .xml .xml #zField
Ms0 @TextInP .responsibility .responsibility #zField
Ms0 @UdInit f0 '' #zField
Ms0 @UdProcessEnd f1 '' #zField
Ms0 @PushWFArc f2 '' #zField
>Proto Ms0 Ms0 MapDialogProcess #zField
Ms0 f0 guid 1790BE16C21C96D0 #txt
Ms0 f0 method start() #txt
Ms0 f0 inParameterDecl '<> param;' #txt
Ms0 f0 outParameterDecl '<> result;' #txt
Ms0 f0 @C|.xml '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<elementInfo>
    <language>
        <name>start()</name>
    </language>
</elementInfo>
' #txt
Ms0 f0 83 51 26 26 -16 15 #rect
Ms0 f1 339 51 26 26 0 12 #rect
Ms0 f2 109 64 339 64 #arcP
>Proto Ms0 .type com.google.map.demo.MapDialog.MapDialogData #txt
>Proto Ms0 .processKind HTML_DIALOG #txt
>Proto Ms0 -8 -8 16 16 16 26 #rect
Ms0 f0 mainOut f2 tail #connect
Ms0 f2 head f1 mainIn #connect
